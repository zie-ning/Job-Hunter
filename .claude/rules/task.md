...
이 문서는 클로드 코드가 이 저장소에서 `docs/tasks/` 아래 Task 파일을 만들고 갱신할 때 참고하는 규칙이다. `docs/ROADMAP.md`는 Phase 단위 상위 체크리스트만 유지하고, Phase의 세부 작업은 이 규칙에 따라 Task 파일로 분리해 관리한다.
...
- 진행 상황을 갱신하는 시점과 근거 확인 규칙은 `CLAUDE.md`의 "개발 워크플로우" 절을 따른다.
